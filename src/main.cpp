...
const char *apPassword = "Peace@2025";
...
// Global variables to hold the current settings, accessible by all functions
...
String formatFrequencyCommand(float freqHz) {
  if (freqHz <= 999) {
    ...
    int tens = (khz / 10) % 10;
    int ones = khz / 10 % 10; // Fixed: was khz % 10, should be (khz / 10) % 10 if you want the second digit
    char buf[10];
...
// ** MODIFIED FUNCTION FOR OLED DISPLAY UPDATES WITH DEBUGGING **
void updateOLEDDisplayParameters() {
    Serial.println("Entering updateOLEDDisplayParameters()"); // Debug

    // Clear the specific area where parameters are displayed
    // Start Y-coordinate (36) is after the "Client connected" line (0, 26) which might appear.
    // If you want it below the IP address (0, 16), 36 is a good starting point.
    // Ensure this rectangle doesn't overlap important static text above it.
    display.fillRect(0, 36, SCREEN_WIDTH, SCREEN_HEIGHT - 36, SSD1306_BLACK); 
    
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 36); // Start displaying parameters from this Y position

    // Print values to Serial Monitor for verification
    Serial.printf("OLED Update: Freq: %.1f Hz\n", currentFreq);
    Serial.printf("OLED Update: Duty: %.0f %%\n", currentDuty);
    Serial.printf("OLED Update: Time: %d min\n", currentTime);

    // Print values to OLED
    display.printf("Freq: %.1f Hz\n", currentFreq);
    display.printf("Duty: %.0f %%\n", currentDuty);
    display.printf("Time: %d min\n", currentTime);
    
    display.display(); // Push changes to the OLED
    Serial.println("Exiting updateOLEDDisplayParameters()"); // Debug
}
...
void setupWebServer() {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    display.display();
    Serial.println("Web client connected (GET request)"); // Debug
  ...
  server.on("/", HTTP_POST, []() {
    Serial.println("Web client submitted form (POST request)"); // Debug
    // Parse and update values
    if (server.hasArg("freq")) {
      currentFreq = server.arg("freq").toFloat();
      Serial.printf("Received Freq: %.1f\n", currentFreq); // Debug
    }
    if (server.hasArg("duty")) {
      currentDuty = server.arg("duty").toFloat();
      Serial.printf("Received Duty: %.0f\n", currentDuty); // Debug
    }
    if (server.hasArg("time")) {
      currentTime = server.arg("time").toInt();
      Serial.printf("Received Time: %d\n", currentTime); // Debug
    }
    ...
    // ** CALL THE NEW OLED UPDATE FUNCTION AFTER VALUES ARE UPDATED **
  ...
  server.onNotFound([]() {
    ...
    Serial.println("404 Not Found"); // Debug
  });
...
void loop() {
  server.handleClient(); // Must be called regularly
}